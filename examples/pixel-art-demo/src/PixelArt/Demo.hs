...
runDemo = runAppHost $ do
  ...
  -- Run application
  runApp win $ demoApp ctx paCtx (V.head scene)
  ...
  setRenderer $ pure $ do
    ...
    let angle2 = time * 0.7
        ...
        shadowLight2 = PointLight
          ...
          , pointLightColor    = V3 0.4 0.8 2.5
          , pointLightPower    = 20
          }
    ...
    let angle3 = time * 1.0
        ...
        shadowLight3 = PointLight
          ...
          , pointLightColor    = V3 0.3 0.9 1.0
          , pointLightPower    = 18
          }
    ...
    let bob = sin (time * 2.0) * 0.5
        ...
        shadowLight4 = PointLight
          ...
          , pointLightColor    = V3 1.0 0.9 0.4
          , pointLightPower    = 16
          }

    -- === Simple fill lights (no shadows) ===

    -- Fill light 1: Soft green from below
    let fillLight1 = PointLight
          { pointLightPosition = V3 0 0 (-1.5)
          , pointLightColor    = V3 0.2 0.8 0.3
          , pointLightPower    = 15
          }
    ...
    let pulse = 0.5 + 0.5 * sin (time * 3.0)
        fillLight2 = PointLight
          ...
          , pointLightColor    = V3 (0.6 * pulse) (0.2 * pulse) (0.8 * pulse)
          , pointLightPower    = 12
          }

    let lighting = MultiLighting
          { multiLightingAmbient      = 0.005
    ...
    executePixelArtPipeline paCtx (pmsWindow ctx)
